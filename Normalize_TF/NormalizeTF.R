...
#
# V 1.0.1  A few stylisitc updates (BS)
# V 1.0    Final lists
#
# ToDo:    All of the imported lists are factors; probably safer to
#          import with stringsAsFactors=FALSE throughout (BS).
#
...
TF2 <- getBM(attributes = c('ensembl_peptide_id', 'hgnc_symbol'),
             filters = 'ensembl_peptide_id',
             values = TF2$EnsemblID,
             mart = ensemblmart)

#TF3 has a permanent link that we can download from, and needs little cleanup.

TF3 <- read.table("http://www.nature.com/nrg/journal/v10/n4/extref/nrg2538-s3.txt",
                  header=TRUE,
                  sep="\t",
                  skip=11,
                  fill=TRUE)
...
TF5 <- read.table("ENCODE_TF_ABDerived.csv",
                  header=TRUE,
                  fill=TRUE,
                  skip=1,
                  sep=",")
...
# Let's change all of the colnames and values to be more descriptive
# and easier to merge - Gene.Symbol is most descriptive, so I'll go with that.
...
#TF1's "Gene.ID" column is meant to be NCBI ID, but since we expanded
# it all the way down to allow for easy merge when constructing the
# table, the IDs may not necessarily be correct. We'll remove this
# from the final table.

#For now (v0.1), I'll make two sets of tables - one including relevant
# information so we have access to it if we want it, and one with just
# the transcription factor names. We'll merge both sets.
...
TFList <- unique(MergedList[ MergedList$Count=="2" | 
                             MergedList$Count=="3" |
                             MergedList$Count=="4" |
                             MergedList$Count=="5", ])$Gene.Symbol

# BS > As far as idiomatic R goes, there is _room_for_improvement_
# BS > in the code above  :-)
